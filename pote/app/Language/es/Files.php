<?php

return [
  "Menu" => "Menú",
  "Dashboard" => "Tablero",
  "Layouts" => "Diseños",
  "Vertical" => "Vertical",
  "Compact_Sidebar" => "Barra lateral compacta",
  "Icon_Sidebar" => "Barra lateral de iconos",
  "Horizontal" => "Horizontal",
  "Topbar_Light" => "Luz de barra superior",
  "Boxed_Layout" => "Diseño en caja",
  "Preloader" => "Precargador",
  "Calendar" => "Calendario",
  "Email" => "Correo electrónico",
  "Inbox" => "Bandeja de entrada",
  "Read_Email" => "Leer el correo electrónico",
  "Tasks" => "Tareas",
  "Task_List" => "Lista de tareas",
  "Kanban_Board" => "Tablero Kanban",
  "Create_Task" => "Crear tarea",
  "Pages" => "Páginas",
  "Login" => "Acceso",
  "Register" => "Registrarse",
  "Recover_Password" => "Recuperar contraseña",
  "Lock_Screen" => "Bloquear pantalla",
  "Starter_Page" => "Página de inicio",
  "Invoice" => "Factura",
  "Profile" => "Perfil",
  "Maintenance" => "Mantenimiento",
  "Coming_Soon" => "Próximamente",
  "Timeline" => "Cronología",
  "FAQs" => "Preguntas frecuentes",
  "Pricing" => "Precios",
  "Error_404" => "error 404",
  "Error_500" => "Error 500",
  "Components" => "Componentes",
  "UI_Elements" => "Elementos de la interfaz de usuario",
  "Alerts" => "Alertas",
  "Buttons" => "Botones",
  "Cards" => "Tarjetas",
  "Carousel" => "Carrusel",
  "Dropdowns" => "Listas deplegables",
  "Grid" => "Cuadrícula",
  "Images" => "Imagenes",
  "Lightbox" => "Caja ligera",
  "Modals" => "Modales",
  "Range_Slider" => "Control deslizante de rango",
  "Session_Timeout" => "Hora de término de la sesión",
  "Progress_Bars" => "Barras de progreso",
  "Sweet_Alert" => "Alerta dulce",
  "Tabs_and_Accordions" => "Pestañas y acordeones",
  "Typography" => "Tipografía",
  "Video" => "Video",
  "General" => "General",
  "Colors" => "Colores",
  "Rating" => "Clasificación",
  "Forms" => "Formularios",
  "Form_Elements" => "Elementos de formulario",
  "Form_Validation" => "Validación de formulario",
  "Form_Advanced" => "Formulario avanzado",
  "Form_Editors" => "Editores de formularios",
  "Form_File_Upload" => "Carga de archivo de formulario",
  "Form_Xeditable" => "Formulario Xeditable",
  "Form_Repeater" => "Repetidor de formulario",
  "Form_Wizard" => "Asistente de formulario",
  "Form_Mask" => "Máscara de forma",
  "Tables" => "Mesas",
  "Basic_Tables" => "Tablas básicas",
  "Data_Tables" => "Tablas de datos",
  "Responsive_Table" => "Tabla receptiva",
  "Editable_Table" => "Tabla editable",
  "Charts" => "Gráficos",
  "Apex_charts" => "Gráficos de Apex",
  "Chartjs_Chart" => "Gráfico de Chartjs",
  "Flot_Chart" => "Gráfico flotante",
  "Jquery_Knob_Chart" => "Gráfico de perillas de Jquery",
  "Sparkline_Chart" => "Gráfico de minigráfico",
  "Icons" => "Iconos",
  "Boxicons" => "Boxicones",
  "Material_Design" => "Diseño de materiales",
  "Dripicons" => "Dripicons",
  "Font_awesome" => "Fuente impresionante",
  "Maps" => "Mapas",
  "Google_Maps" => "mapas de Google",
  "Vector_Maps" => "Mapas vectoriales",
  "Multi_Level" => "Multi nivel",
  "Level_1_1" => "Nivel 1.1",
  "Level_1_2" => "Nivel 1.2",
  "Level_2_1" => "Nivel 2.1",
  "Level_2_2" => "Nivel 2.2",
  "Notifications" => "Notificaciones",
  "View_All" => "Ver todo",
  "Your_order_is_placed" => "Se realiza tu pedido",
  "If_several_languages_coalesce_the_grammar" => "Si varios idiomas fusionan la gramática",
  "min_ago" => "Hace 3 minutos",
  "It_will_seem_like_simplified_English" => "Parecerá un inglés simplificado.",
  "hours_ago" => "Hace 1 hora",
  "Your_item_is_shipped" => "Tu artículo es enviado",
  "As_a_skeptical_Cambridge_friend_of_mine_occidental" => "Como escéptico amigo mío occidental de Cambridge.",
  "View_More" => "Ver más..",
  "My_Wallet" => "Mi billetera",
  "Settings" => "Ajustes",
  "Lock_screen" => "Bloquear pantalla",
  "Logout" => "Cerrar sesión",
  "Search" => "Buscar...",
  "Mega_Menu" => "Mega menú",
  "UI_Components" => "Componentes de UI",
  "Applications" => "Aplicaciones",
  "Ecommerce" => "Comercio electrónico",
  "Projects" => "Proyectos",
  "Contacts" => "Contactos",
  "Extra_Pages" => "Páginas extra",
  "Light_Sidebar" => "Barra lateral ligera",
  "Horizontal_layout" => "Disposición horizontal",
  "Time" => "Hora",
  "Sale" => "Venta",
  "Up_to" => "Hasta",
  "Off" => "Apagado",
  "At_vero_eos_accusamus_et_iusto_odio" => "En vero eos accusamus et iusto odio.",
  "View_more" 	=>	 "Ver más",
  "Buy_more" 	=>	 "Comprar más",
  "Transations_coins" 	=>	 "Transacciones de Monedas",
  "Apps" 	=>	 "Aplicaciones",
  "Personal_data" 	=>	 "Datos personales",
  "Reviews" 	=>	 "Reseñas",
  "Ranking" 	=>	 "Clasificación",
  "Financial" 	=>	 "Financiero",
  "Buy_coins" 	=>	"Comprar Monedas",
  "Transfer_coins" 	=>	"Transferir Monedas",
  "Withdraw" 	=>	"Retirar",
  "Conversion_rates" 	=>	"Tasas de conversión",
  "Extract" 	=>	"Extraer",
  "Purchases" 	=>	"Compras",
  "Coins" 	=>	"Monedas",
  "Transfers" 	=>	"Transferencias",
  "Evaluate_now" 	=>	 "Evaluar ahora",
  "Is_website_right" 	=>	 "¿Es precisa la sugerencia del sitio?",
  "Last_Ranking" 	=>	 "Última clasificación",
  "Lasts_results" 	=>	 "Últimos resultados",
  "Event" 	=>	 "Evento",
  "Website_opinion" 	=>	 "Consejo sobre el sitio web",
  "Suport" => "Contacto",
  "Yes" => "Sí",
  "No" => "No",
    "Pot_of_Coins" =>"Olla de monedas",
    "How_it_works"=>"¿Como funciona?",
  "Results" 	=>	 "Resultados"
];

?>