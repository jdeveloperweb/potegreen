...
if ($conf == '1') {
	...
	while($linhas3 = mysqli_fetch_array($resultado2)){
		...
		if ($soma == $conta) {
			mysqli_query($conectar, "UPDATE multiplassug set status = 'Vencida', retorno = 1 WHERE  id='$idm'");;
		} else {
...
<body>
	...
	if (mysqli_affected_rows($conectar) != 0) {
		echo "
				<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=https://multiplagreen.com/user/administrador.php?link=8'>
		...
		echo "
				<META HTTP-EQUIV=REFRESH CONTENT = '0;URL= https://multiplagreen.com/user/administrador.php?link=8'>
				<script type=\"text/javascript\">
					alert(\"Avaliação de jogo cadastrada com sucesso!!! Verifique!!\");
				</script>
			";
	}
