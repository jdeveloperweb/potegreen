...
<div class="container theme-showcase" role="main">      
  ...
  <div class="row">
	...
	<?php
	//Pesquisar o id dos produtos em destaque no nivel 1 página principal
	$resultado_prod_dest_dois=mysql_query("SELECT * FROM destaques_produtos WHERE nivel_dois='1' ORDER BY ordem ASC");
	$linhas_prod_dest_dois=mysql_num_rows($resultado_prod_dest_dois);
	...
	<div class="col-md-12">
	  <table class="table">
		...
		<tbody>
			<?php 
				$contr_ord_dois = 1;
				while($linhas_prod_dest_dois = mysql_fetch_array($resultado_prod_dest_dois)){
					//id do produto na tabela produto em destaque
					$produto_id_dois = $linhas_prod_dest_dois['produto_id'];
					$produto_dest_id_dois = $linhas_prod_dest_dois['id'];
					
					//Selecionar os dados do produto no BD
					$resultado_prod_dois=mysql_query("SELECT * FROM produtos WHERE id='$produto_id_dois' LIMIT 1");
					$linhas_prod_dois=mysql_fetch_assoc($resultado_prod_dois);
					echo "<tr>";
